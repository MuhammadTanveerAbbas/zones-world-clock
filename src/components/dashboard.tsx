		...
		<OverlayPanel open={open} onClose={onClose} title="Dashboard" width="lg">
			<div className="p-4 sm:p-5 space-y-4">
					...
					<div className="h-28 sm:h-32">
						<ResponsiveContainer width="100%" height="100%">
							<BarChart data={chartData} margin={{ top: 4, right: 4, bottom: 0, left: 0 }}>
								<XAxis
									dataKey="day"
									tick={{ fontSize: 9, fontFamily: "var(--font-mono)", fill: "var(--muted-fg)" }}
									axisLine={false}
									tickLine={false}
								/>
								<Tooltip
									contentStyle={{ background: "var(--bg)", border: "1px solid var(--border)", borderRadius: "8px", fontSize: "10px", fontFamily: "var(--font-mono)" }}
									labelStyle={{ color: "var(--fg)" }}
								/>
								<Bar dataKey="focus" fill="var(--fg)" radius={[3, 3, 0, 0]} maxBarSize={32} opacity={0.8} />
							</BarChart>
						</ResponsiveContainer>
					</div>
				)}
				...
				<div className="flex items-center justify-between border-t border-(--color-border) pt-3">
					...
					>
						{showSessions ? "hide session history" : `session history (${recentSessions.length})`}
					</button>
					...
					<div className="max-h-40 overflow-y-auto space-y-0.5">
						{recentSessions.map((s) => (
							<div key={s.id} className="flex items-center justify-between font-mono text-[9px] px-2 py-1 rounded hover:bg-(--color-foreground)/[0.03]">
								...
								<span className="tabular-nums text-(--color-muted-foreground)">{formatDuration(s.duration)}</span>
							</div>
						))}
					</div>
				)}
